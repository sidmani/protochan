...
namespace SIMD {
  var IV512 = Helper.bytes2Int32Buffer(Helper.b64Decode("C6FrlXL5ma2f7MKuujJk/F6JSSmOnzDlLx2qN/DyxVisUGZDqQY1peJbh4uqt4ePiIF/egoCiStVmnVQWY9lfn7vYKFrcOPonBcU0blY4qirAmde7RwBT82NZbv9t6JXCSVImdaZx7yQGbbcK5Ai5I+hSVYhv5vTuU0JQ2/93CI="));
  /*
  ...
  function FFT_LOOP(q, qOffset, hk, as) {
    var u, v;
    var m = q[(qOffset)];
    var n = q[(qOffset) + (hk)];
    q[(qOffset)] = m + n;
  ...
  function FFT16(x, xOffset, q, qOffset, xs) {
    var d1 = new Array(8);
    var d2 = new Array(8);
  ...
  function FFT32(x, xOffset, q, qOffset, xs) {
    var xd = xs << 1;
  ...
  function FFT64(x, xOffset, q, qOffset, xs) {
    var xd = xs << 1;
  ...
  function FFT256(x, xOffset, q, qOffset, xs) {
  ...
  function INNER(l, h, mm) {
    return (((l * mm) & 0xFFFF) + ((h * mm) << 16));
  }

  function W_BIG(sb, o1, o2, mm, q) {
    var r = new Array(8);
    for (var i = 0;i<8;i++) {
  ...
  function STEP_ELT(n, w, fun, s, ppb, tA, A, B, C, D) {
  ...
  function STEP_BIG(w, fun, r, s, pp8b, A, B, C, D) {
    var tA = new Array(8);
    for (var i = 0;i<8;i++) {
  ...
  function ONE_ROUND_BIG(ri, isp, p0, p1, p2, p3, A, B, C, D, q) {
  ...
  function compress(ctx, last) {
    ...
    var x = Op.swap32Array(Helper.bytes2Int32Buffer(ctx.buffer));
  ...
  function encode_count(dst, offset, low, high, ptr, n) {
    low = Op.t32(low << 10);
